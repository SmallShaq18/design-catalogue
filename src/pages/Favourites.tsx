...
import { Link } from "react-router-dom";
import { Heart } from "lucide-react";
import { GalleryGrid, LS_FAV } from "../Components/gallery/GalleryGrid";
import { ImageLightbox } from "../Components/gallery/ImageLightbox";
import { projects } from "../data/projects";
...
export const Favourites = () => {
  ...
  const [favImgs, setFavImgs] = useState<string[]>([]);
  const [lightboxOpen, setLightboxOpen]   = useState(false);
  const [lightboxIndex, setLightboxIndex] = useState(0);
  const [tab, setTab] = useState<"projects" | "images">("projects");
  ...
  // Read favourites — project IDs are short numeric strings; image URLs start with "http"
  useEffect(() => {
    const all = readLS(LS_FAV);
    setFavIds(all.filter((x) => !x.startsWith("http")));
    setFavImgs(all.filter((x) => x.startsWith("http")));
  }, []);

  const favProjects = projects.filter((p) => favIds.includes(p.id));

  const removeFavImg = (url: string) => {
    setFavImgs((prev) => {
      const next = prev.filter((x) => x !== url);
      // keep project ids intact
      const currentAll = readLS(LS_FAV);
      writeLS(LS_FAV, currentAll.filter((x) => x !== url));
      return next;
    });
  };
  ...
  const isEmpty = favProjects.length === 0 && favImgs.length === 0;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f0ea" }}>
      <div style={{ maxWidth: 1440, margin: "0 auto", padding: "3%" }}>
        ...
        <div style={{ marginBottom: "clamp(3rem,5vw,4.5rem)" }}>
          <div style={fadeIn(0)}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingBottom: "0.7rem" }}>
              ...
              <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "0.58rem",
                letterSpacing: "0.18em", color: "rgba(10,9,8,0.3)" }}>
                {favProjects.length + favImgs.length} Items
              </span>
            </div>
            <div style={{ height: 1, background: "rgba(10,9,8,0.1)" }} />
          </div>
          ...
          <h1 style={{
            fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 700,
            fontSize: "clamp(3rem,10vw,8rem)", lineHeight: 0.9, letterSpacing: "-0.03em",
            color: "#0a0908", margin: "clamp(1.5rem,3vw,2.5rem) 0 0", ...fadeIn(0.08),
          }}>
            Favourites
          </h1>
        </div>
        ...
        {isEmpty ? (
          <div style={{ padding: "5rem 0", textAlign: "center", ...fadeIn(0.15) }}>
            <Heart size={28} style={{ color: "rgba(10,9,8,0.15)", margin: "0 auto 1.5rem" }} />
            <p style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontStyle: "italic",
              fontSize: "1.4rem", color: "rgba(10,9,8,0.35)", marginBottom: "1.5rem" }}>
              Nothing saved yet.
            </p>
            <Link to="/projects" style={{
              ...
              textTransform: "uppercase" as const, color: "#0a0908", textDecoration: "none",
              borderBottom: "1px solid rgba(10,9,8,0.3)", paddingBottom: "2px",
            }}>
              Browse Work →
            </Link>
          </div>
          ...
          <>
            {/* Tab switcher — only show if both buckets have items *
            {favProjects.length > 0 && favImgs.length > 0 && (
              <div style={{ display: "flex", gap: "0.45rem", marginBottom: "2rem", ...fadeIn(0.15) }}>
                {(["projects", "images"] as const).map((t) => (
                  <button key={t} onClick={() => setTab(t)} style={{
                    fontFamily: "'DM Mono', monospace", fontSize: "0.55rem",
                    letterSpacing: "0.18em", textTransform: "uppercase" as const,
            ...
            <div style={fadeIn(0.2)}>
              {/* Saved projects *
              {(tab === "projects" || favImgs.length === 0) && favProjects.length > 0 && (
                <GalleryGrid projects={favProjects} hideToolbar={false} />
              )}

              {/* Saved individual images *
              {(tab === "images" || favProjects.length === 0) && favImgs.length > 0 && (
                <div>
...
// ─── Single saved-image card ──────────────────────────────────────────────────
const SavedImageCard = ({
  url, onOpen, onRemove,
}: { url: string; onOpen: () => void; onRemove: () => void }) => {
  const [hovered, setHovered] = useState(false);
